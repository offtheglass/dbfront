...
function Joinonetableresult() {
  ...
  const columns = [
    ...
    useEffect(() => {
        ...
        fetch('http://165.132.129.99:8080/getonejoinedtable' ,{
        method:"GET",
    }).then(response=>{ 
        console.log(response);return response.json()
}).then(response=>{
  console.log(response["joinResult"])
  var temp=response["joinResult"]
  temp["id"]=1
  Setinformation([temp])
  console.log(Object.keys(temp))
  console.log(Object.values(temp))
  var temp2=[]
  for(var o in Object.values(temp)){
      temp2.push(String(Object.values(temp)[o]))
    }
    csvdata.push(temp2)
    console.log(csvdata)
...
export default Joinonetableresult